...
static int32_t hmac_aead_derive_b_enc_b_auth(const uint8_t *iv, uint8_t *b_enc, uint8_t *b_auth)
{
    ...
    // Check for domain separation. It will make sure IV is NOT all 00s, FFs or palindrome
    // a typical implemenation isse
    if (cmn_memcmp(b_enc, b_auth, SIZE_IV) == 0)
    {
        return HMAC_AEAD_INVALID_IV;
    }
                                  ...
                                  size_t *padded_ciphertext_len)
{
    ...
    *padded_ciphertext_len = ciphertext_len;
    ...
    else
    {
        ...
        return HMAC_AEAD_AUTH_GEN_FAILED;
    }
                      ...
                      size_t enc_msg_buf_len, size_t *enc_msg_len, uint8_t *iv, uint8_t *tag)
{
    ...
    *enc_msg_len = 0;
    ...
    {
        return HMAC_AEAD_ERR_BUF_TOO_SMALL;
    }

    // Generate random IV
    if (cmn_secure_random(iv, SIZE_IV) != COMMON_OK)
    {
        status = HMAC_AEAD_GENERAL_FAILURE;
        goto cleanup;
    }
    ...
    if (hmac_aead_encrypt_decrypt(msg, msg_len, key_enc, iv, enc_msg_buf) != HMAC_AEAD_OK)
    {
        status = HMAC_AEAD_ENCRYPTION_FAILED;
        goto cleanup;
    }

    /********* Generate Tag **********/

    if (hmac_aead_gen_auth(key_auth, aad, aad_len, enc_msg_buf, msg_len, tag, enc_msg_len) != HMAC_AEAD_OK)
    {
        status = HMAC_AEAD_AUTH_GEN_FAILED;
        goto cleanup;
    }
                                     ...
                                     size_t *len_nopad_ciphertext)
{
    ...
    {
        return HMAC_AEAD_INVALID_CIPHER_TEXT_LEN;
    }
    ...
    {
        return HMAC_AEAD_INVALID_ADD_LEN;
    }
    ...
    {
        return HMAC_AEAD_AUTH_VERIFY_FAILED;
    }
                      ...
                      const uint8_t *iv, uint8_t *dec_msg, size_t *dec_msg_len)
{
    ...
    if ((hmac_aead_verify_auth(enc_msg, enc_msg_len, aad, aad_len, key_auth, tag, &msg_len)) != HMAC_AEAD_OK)
    {
        return HMAC_AEAD_AUTH_VERIFY_FAILED;
    }
    ...
    if (hmac_aead_encrypt_decrypt(enc_msg, msg_len, key_dec, iv, dec_msg) != HMAC_AEAD_OK)
    {
        // Clear encryption and authentication keys
        cmn_memset(key_dec, 0, SIZE_ENC_KEY);

        return HMAC_AEAD_ENCRYPTION_FAILED;
    }
